  ...
  <div class="ratingselect">
    <div class="rating-type border-1px">
      <span class="block" :class="{active:selectType===2}" @click="setSelectType(2)">
        全部
        <span class="count">{{totalRatingNum}}</span>
      </span>
      <span class="block" :class="{active:selectType===0}" @click="setSelectType(0)">
        推荐
        <span class="count">{{totalGoodRatingNum}}</span>
      </span>
      <span class="block" :class="{active:selectType===1}" @click="setSelectType(1)">
        吐槽
        <span class="count">{{totalRatingNum - totalGoodRatingNum}}</span>
      </span>
    </div>
...
export default {
  ...
  computed: {
    ...mapGetters(['totalRatingNum', 'totalGoodRatingNum'])
  },
  methods: {
    setSelectType (type) {
      // 分发vue自定义事件
